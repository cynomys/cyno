...
fn create_batch_quads<'a>(kmers: &Vec<&str>, hm: &mut HashMap<String, String>, genome_name: &str) -> Vec<String>{
...
//    println!("{:?}", mutation);
    ...
    match m {
        Ok(m) => m,
        Err(m) => return Err(Error::new(ErrorKind::Other, format!("Failed to insert NQuads. {}", m)))
    };
...
fn query_batch_dgraph(client: &Dgraph, hmc: &mut HashMap<String, String>, kmers: &Vec<&str>) ->Result<(), Error>{
